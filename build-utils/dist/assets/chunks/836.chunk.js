(self["webpackChunkinterface_white_blue_swaps"] = self["webpackChunkinterface_white_blue_swaps"] || []).push([[836],{
...
var browser = __webpack_require__(5905);
...
function themeModeVariables() {
  var _a;
  const themeMode = (_a = dist/* ThemeCtrl.state.themeMode */.Ic.state.themeMode) != null ? _a : "dark";
...
const ThemeUtil = {
  ...
  setTheme() {
    ...
    const { themeVariables } = dist/* ThemeCtrl.state */.Ic.state;
    if (root) {
...
let WcmModalBackcard = class extends lit_element_s {
  // -- render ------------------------------------------------------- //
  render() {
    return x`<div class="wcm-toolbar-placeholder"></div><div class="wcm-toolbar">${SvgUtil.WALLET_CONNECT_LOGO} <button @click="${dist/* ModalCtrl.close */.jb.close}">${SvgUtil.CROSS_ICON}</button></div>`;
  }
};
...
let WcmModalHeader = class extends lit_element_s {
  ...
  // -- private ------------------------------------------------------ //
  backBtnTemplate() {
    return x`<button class="wcm-back-btn" @click="${dist/* RouterCtrl.goBack */.AV.goBack}">${SvgUtil.BACK_ICON}</button>`;
  }
  actionBtnTemplate() {
  ...
  render() {
    ...
    const backBtn = dist/* RouterCtrl.state.history.length */.AV.state.history.length > 1;
...
const UiUtil = {
  ...
  getWalletIcon({ id, image_id }) {
    const { walletImages } = dist/* ConfigCtrl.state */.t0.state;
    if (walletImages == null ? void 0 : walletImages[id]) {
      return walletImages[id];
    } else if (image_id) {
      return dist/* ExplorerCtrl.getWalletImageUrl */.uc.getWalletImageUrl(image_id);
    }
    return "";
  },
  ...
  async preloadImage(src) {
    ...
    return Promise.race([imagePromise, dist/* CoreUtil.wait */.zv.wait(3e3)]);
  },
  getErrorMessage(err) {
  ...
  handleMobileLinking(wallet, target = "_self") {
    const { walletConnectUri } = dist/* OptionsCtrl.state */.zb.state;
    ...
    function onRedirect(uri) {
      if (nativeUrl) {
        const href = dist/* CoreUtil.formatNativeUrl */.zv.formatNativeUrl(nativeUrl, uri, name);
        dist/* CoreUtil.openHref */.zv.openHref(href, target);
      } else if (universalUrl) {
        const href = dist/* CoreUtil.formatUniversalUrl */.zv.formatUniversalUrl(universalUrl, uri, name);
        dist/* CoreUtil.openHref */.zv.openHref(href, target);
      }
    }
    if (walletConnectUri) {
  ...
  handleAndroidLinking() {
    const { walletConnectUri } = dist/* OptionsCtrl.state */.zb.state;
    if (walletConnectUri) {
      dist/* CoreUtil.setWalletConnectAndroidDeepLink */.zv.setWalletConnectAndroidDeepLink(walletConnectUri);
      dist/* CoreUtil.openHref */.zv.openHref(walletConnectUri, dist/* CoreUtil.isTelegram */.zv.isTelegram() ? "_blank" : "_self");
    }
  },
  async handleUriCopy() {
    const { walletConnectUri } = dist/* OptionsCtrl.state */.zb.state;
    if (walletConnectUri) {
      try {
        ...
        dist/* ToastCtrl.openToast */.Vs.openToast("Link copied", "success");
      } catch (e) {
        dist/* ToastCtrl.openToast */.Vs.openToast("Failed to copy", "error");
      }
    }
  },
  getCustomImageUrls() {
  ...
  openWalletExplorerUrl() {
    dist/* CoreUtil.openHref */.zv.openHref(UiUtil.EXPLORER_WALLET_URL, "_blank");
  },
  getCachedRouterWalletPlatforms() {
    const { desktop, mobile } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
  ...
  goToConnectingView(wallet) {
    dist/* RouterCtrl.setData */.AV.setData({ Wallet: wallet });
    const isMobileDevice = dist/* CoreUtil.isMobile */.zv.isMobile();
    const { isDesktop, isWeb, isMobile } = UiUtil.getCachedRouterWalletPlatforms();
    if (isMobileDevice) {
      if (isMobile) {
        dist/* RouterCtrl.push */.AV.push("MobileConnecting");
        if (!dist/* CoreUtil.isAndroid */.zv.isAndroid() && dist/* CoreUtil.isTelegram */.zv.isTelegram()) {
          this.handleMobileLinking(wallet, "_blank");
        }
      } else if (isWeb) {
        dist/* RouterCtrl.push */.AV.push("WebConnecting");
      } else {
        dist/* RouterCtrl.push */.AV.push("InstallWallet");
      }
    } else if (isDesktop) {
      dist/* RouterCtrl.push */.AV.push("DesktopConnecting");
    } else if (isWeb) {
      dist/* RouterCtrl.push */.AV.push("WebConnecting");
    } else if (isMobile) {
      dist/* RouterCtrl.push */.AV.push("MobileQrcodeConnecting");
    } else {
      dist/* RouterCtrl.push */.AV.push("InstallWallet");
    }
  }
...
let WcmModalRouter = class extends lit_element_s {
  ...
  constructor() {
    super();
    this.view = dist/* RouterCtrl.state.view */.AV.state.view;
    this.prevView = dist/* RouterCtrl.state.view */.AV.state.view;
    // -- private ------------------------------------------------------ //
    this.unsubscribe = void 0;
    this.oldHeight = "0px";
    this.resizeObserver = void 0;
    this.unsubscribe = dist/* RouterCtrl.subscribe */.AV.subscribe((routerState) => {
...
let WcmModalToast = class extends lit_element_s {
  constructor() {
    ...
    this.unsubscribe = void 0;
    this.timeout = void 0;
    this.unsubscribe = dist/* ToastCtrl.subscribe */.Vs.subscribe((newState) => {
      if (newState.open) {
        this.open = true;
        this.timeout = setTimeout(() => dist/* ToastCtrl.closeToast */.Vs.closeToast(), 2200);
      } else {
        this.open = false;
  ...
  disconnectedCallback() {
    ...
    (_a = this.unsubscribe) == null ? void 0 : _a.call(this);
    clearTimeout(this.timeout);
    dist/* ToastCtrl.closeToast */.Vs.closeToast();
  }
  // -- render ------------------------------------------------------- //
  render() {
    const { message, variant } = dist/* ToastCtrl.state */.Vs.state;
    const classes = {
...
let WcmQrCode = class extends lit_element_s {
  ...
  // -- private ------------------------------------------------------ //
  svgTemplate() {
    const isLightMode = dist/* ThemeCtrl.state.themeMode */.Ic.state.themeMode === "light";
  ...
  render() {
    const classes = {
      "wcm-dark": dist/* ThemeCtrl.state.themeMode */.Ic.state.themeMode === "dark"
    };
...
let WcmWalletButton = class extends lit_element_s {
  ...
  handleClick() {
    dist/* EventsCtrl.click */.uA.click({ name: "WALLET_BUTTON", walletId: this.walletId });
    this.onClick();
  }
  // -- render ------------------------------------------------------- //
  render() {
...
let WcmExplorerContext = class extends lit_element_s {
  ...
  async preloadListings() {
    if (dist/* ConfigCtrl.state.enableExplorer */.t0.state.enableExplorer) {
      await dist/* ExplorerCtrl.getRecomendedWallets */.uc.getRecomendedWallets();
      dist/* OptionsCtrl.setIsDataLoaded */.zb.setIsDataLoaded(true);
      const { recomendedWallets } = dist/* ExplorerCtrl.state */.uc.state;
  ...
  async preloadData() {
    try {
      ...
      console.error(err);
      dist/* ToastCtrl.openToast */.Vs.openToast("Failed preloading", "error");
    }
  }
};
__decorateClass$i([
...
let WcmThemeContext = class extends lit_element_s {
  ...
  constructor() {
    ...
    ThemeUtil.setTheme();
    this.unsubscribeTheme = dist/* ThemeCtrl.subscribe */.Ic.subscribe(ThemeUtil.setTheme);
  }
  disconnectedCallback() {
    var _a;
...
let WcmAndroidWalletSelection = class extends lit_element_s {
  // -- private ------------------------------------------------------ //
  onGoToQrcode() {
    dist/* RouterCtrl.push */.AV.push("Qrcode");
  }
  // -- render ------------------------------------------------------- //
  render() {
    const { recomendedWallets } = dist/* ExplorerCtrl.state */.uc.state;
    const wallets = [...recomendedWallets, ...recomendedWallets];
    const recomendedCount = dist/* CoreUtil.RECOMMENDED_WALLET_AMOUNT */.zv.RECOMMENDED_WALLET_AMOUNT * 2;
...
let WcmConnectorWaiting = class extends lit_element_s {
  ...
  svgLoaderTemplate() {
    ...
    const DH_ARRAY = 317;
    const DH_OFFSET = 425;
    const radius = (_b = (_a = dist/* ThemeCtrl.state.themeVariables */.Ic.state.themeVariables) == null ? void 0 : _a["--wcm-wallet-icon-large-border-radius"]) != null ? _b : ThemeUtil.getPreset("--wcm-wallet-icon-large-border-radius");
    let numRadius = 0;
    if (radius.includes("%")) {
...
const DataUtil = {
  manualWallets() {
    var _a, _b;
    const { mobileWallets, desktopWallets } = dist/* ConfigCtrl.state */.t0.state;
    const recentWalletId = (_a = DataUtil.recentWallet()) == null ? void 0 : _a.id;
    const platformWallets = dist/* CoreUtil.isMobile */.zv.isMobile() ? mobileWallets : desktopWallets;
    const wallets = platformWallets == null ? void 0 : platformWallets.filter((wallet) => recentWalletId !== wallet.id);
    return (_b = dist/* CoreUtil.isMobile */.zv.isMobile() ? wallets == null ? void 0 : wallets.map(({ id, name, links }) => ({ id, name, mobile: links, links })) : wallets == null ? void 0 : wallets.map(({ id, name, links }) => ({ id, name, desktop: links, links }))) != null ? _b : [];
  },
  recentWallet() {
...
let WcmDesktopWalletSelection = class extends lit_element_s {
  // -- render ------------------------------------------------------- //
  render() {
    const { explorerExcludedWalletIds, enableExplorer } = dist/* ConfigCtrl.state */.t0.state;
...
let WcmLegalNotice = class extends lit_element_s {
  // -- render ------------------------------------------------------- //
  render() {
    const { termsOfServiceUrl, privacyPolicyUrl } = dist/* ConfigCtrl.state */.t0.state;
...
let WcmModal = class extends lit_element_s {
  ...
  constructor() {
    ...
    this.unsubscribeModal = dist/* ModalCtrl.subscribe */.jb.subscribe((modalState) => {
      if (modalState.open) {
...
let WcmPlatformSelection = class extends lit_element_s {
  ...
  // -- private ------------------------------------------------------ //
  onMobile() {
    const isMobile = dist/* CoreUtil.isMobile */.zv.isMobile();
    if (isMobile) {
      dist/* RouterCtrl.replace */.AV.replace("MobileConnecting");
    } else {
      dist/* RouterCtrl.replace */.AV.replace("MobileQrcodeConnecting");
    }
  }
  onDesktop() {
    dist/* RouterCtrl.replace */.AV.replace("DesktopConnecting");
  }
  onWeb() {
    dist/* RouterCtrl.replace */.AV.replace("WebConnecting");
  }
  // -- render ------------------------------------------------------- //
  render() {
...
let WcmViewAllWalletsButton = class extends lit_element_s {
  // -- render ------------------------------------------------------- //
  onClick() {
    dist/* RouterCtrl.push */.AV.push("WalletExplorer");
  }
  // -- render ------------------------------------------------------- //
  render() {
    const { recomendedWallets } = dist/* ExplorerCtrl.state */.uc.state;
...
let WcmConnectWalletView = class extends lit_element_s {
  ...
  viewTemplate() {
    ...
    if (dist/* CoreUtil.isMobile */.zv.isMobile()) {
...
let WcmDesktopConnectingView = class extends lit_element_s {
  ...
  onFormatAndRedirect(uri) {
    const { desktop, name } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
    const nativeUrl = desktop == null ? void 0 : desktop.native;
    const universalUrl = desktop == null ? void 0 : desktop.universal;
    if (nativeUrl) {
      const href = dist/* CoreUtil.formatNativeUrl */.zv.formatNativeUrl(nativeUrl, uri, name);
      dist/* CoreUtil.openHref */.zv.openHref(href, "_self");
    } else if (universalUrl) {
      const href = dist/* CoreUtil.formatUniversalUrl */.zv.formatUniversalUrl(universalUrl, uri, name);
      dist/* CoreUtil.openHref */.zv.openHref(href, "_blank");
    }
  }
  openDesktopApp() {
    const { walletConnectUri } = dist/* OptionsCtrl.state */.zb.state;
    const routerData = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
  ...
  // -- render ------------------------------------------------------- //
  render() {
    const { name, id, image_id } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
...
let WcmInstallWalletView = class extends lit_element_s {
  ...
  onInstall(uri) {
    if (uri) {
      dist/* CoreUtil.openHref */.zv.openHref(uri, "_blank");
    }
  }
  // -- render ------------------------------------------------------- //
  render() {
    const { name, id, image_id, homepage } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
...
let WcmMobileConnectingView = class extends lit_element_s {
  ...
  onFormatAndRedirect(uri, forceUniversalUrl = false) {
    const { mobile, name } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
    const nativeUrl = mobile == null ? void 0 : mobile.native;
    const universalUrl = mobile == null ? void 0 : mobile.universal;
    const target = dist/* CoreUtil.isTelegram */.zv.isTelegram() ? "_blank" : "_self";
    uri = dist/* CoreUtil.isTelegram */.zv.isTelegram() && dist/* CoreUtil.isAndroid */.zv.isAndroid() ? encodeURIComponent(uri) : uri;
  ...
  onGoToAppStore(downloadUrl) {
    if (downloadUrl) {
      dist/* CoreUtil.openHref */.zv.openHref(downloadUrl, "_blank");
    }
  }
  // -- render ------------------------------------------------------- //
  render() {
    const { name, id, image_id, app, mobile } = dist/* CoreUtil.getWalletRouterData */.zv.getWalletRouterData();
...
let WcmWalletExplorerView = class extends lit_element_s {
  constructor() {
    super(...arguments);
    this.loading = !dist/* ExplorerCtrl.state.wallets.listings.length */.uc.state.wallets.listings.length;
    this.firstFetch = !dist/* ExplorerCtrl.state.wallets.listings.length */.uc.state.wallets.listings.length;
    this.search = "";
    ...
    this.searchDebounce = UiUtil.debounce((value) => {
      if (value.length >= 1) {
        ...
        this.endReached = false;
        this.search = value;
        dist/* ExplorerCtrl.resetSearch */.uc.resetSearch();
        this.fetchWallets();
      } else if (this.search) {
        this.search = "";
  ...
  isLastPage() {
    const { wallets, search } = dist/* ExplorerCtrl.state */.uc.state;
  ...
  async fetchWallets() {
    ...
    if (!this.endReached && (this.firstFetch || total > PAGE_ENTRIES && listings.length < total)) {
      try {
        this.loading = true;
        const chains = (_a = dist/* OptionsCtrl.state.chains */.zb.state.chains) == null ? void 0 : _a.join(",");
        const { listings: newListings } = await dist/* ExplorerCtrl.getWallets */.uc.getWallets({
        ...
        await Promise.all([
          ...
          dist/* CoreUtil.wait */.zv.wait(300)
        ]);
        this.endReached = this.isLastPage();
      } catch (err) {
        console.error(err);
        dist/* ToastCtrl.openToast */.Vs.openToast(UiUtil.getErrorMessage(err), "error");
      } finally {
        this.loading = false;
  ...
  onConnect(listing) {
    if (dist/* CoreUtil.isAndroid */.zv.isAndroid()) {
...
let WcmWebConnectingView = class extends lit_element_s {
  ...
  render() {
    ...
    const isMobilePlatform = dist/* CoreUtil.isMobile */.zv.isMobile();
